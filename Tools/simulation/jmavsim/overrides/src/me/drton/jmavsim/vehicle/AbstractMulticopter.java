...
public abstract class AbstractMulticopter extends AbstractVehicle {
    ...
    private TransformGroup[] rotorModelGroups;
    private Vector3d[] rotorModelPositions;
    private double[] rotorModelAngles;
    private long lastRotorModelTime = -1;
    ...
    public void enableRotorModelVisualization(String[] modelNames, Vector3d[] positions) {
        if (transformGroup == null || modelNames.length != rotors.length || positions.length != rotors.length) {
            return;
        }

        rotorModelGroups = new TransformGroup[rotors.length];
        rotorModelPositions = new Vector3d[rotors.length];
        rotorModelAngles = new double[rotors.length];

        for (int i = 0; i < rotors.length; i++) {
            rotorModelPositions[i] = new Vector3d(positions[i]);
            Transform3D transform = new Transform3D();
            transform.setTranslation(rotorModelPositions[i]);
            rotorModelGroups[i] = new TransformGroup(transform);
            rotorModelGroups[i].setCapability(TransformGroup.ALLOW_TRANSFORM_WRITE);

            BranchGroup model = loadModelFromFile(modelNames[i]);

            if (model != null) {
                rotorModelGroups[i].addChild(model);
                transformGroup.addChild(rotorModelGroups[i]);
            }
        }
    }

    private void updateRotorModelVisualization(long t) {
        if (rotorModelGroups == null) {
            return;
        }

        double dt = lastRotorModelTime >= 0 ? Math.max((t - lastRotorModelTime) / 1000.0, 0.0) : 0.0;
        lastRotorModelTime = t;

        for (int i = 0; i < rotors.length; i++) {
            double control = Math.abs(rotors[i].getControl());
            double direction = rotors[i].getFullTorque() >= 0.0 ? 1.0 : -1.0;
            rotorModelAngles[i] += direction * control * 60.0 * dt;

            Transform3D rotation = new Transform3D();
            rotation.rotZ(rotorModelAngles[i]);
            rotation.setTranslation(rotorModelPositions[i]);
            rotorModelGroups[i].setTransform(rotation);
        }
    }
    ...
    public void update(long t, boolean paused) {
        ...
        updateRotorModelVisualization(t);
    }
